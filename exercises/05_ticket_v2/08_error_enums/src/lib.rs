...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        ...
        Err(TicketNewError::DescriptionError { error }) => Ticket {
            title: title,
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {
                error: ("Title cannot be longer than 50 bytes".to_string()),
            });
        }
        ...
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError {
                error: ("Description cannot be longer than 500 bytes".to_string()),
            });
        }
